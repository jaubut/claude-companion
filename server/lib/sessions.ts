...
//   3. sessionId    — stable per claude-code invocation
...
// A claude-code session can sit idle for hours between turns (user went AFK,
...
function isClaudePidAlive(pid: string): boolean {
  if (!pid) return false
  const n = Number(pid)
  if (!Number.isFinite(n) || n <= 1) return false
  // `kill -0` only tells us a process with that pid exists — after pid reuse
  // it could be anything. Verify it's still a claude process by reading comm.
  try {
...
function deriveKey(meta: Partial<Session> & { cwd: string }): string {
  ...
  if (meta.iTermSessionId) return `iterm:${meta.iTermSessionId}`
...
export function recordSession(
  ...
  const nextLabel = opts.provisional && !prev?.label
    ? ""
  ...
  if (key.startsWith("tty:")) {
    for (const [otherKey, s] of sessions) {
      ...
      if (otherKey.startsWith("tty:")) continue
