...
## server (bun-server, `server/`, 62 modules, cap 600)
...
| `lib/activity.ts` | 480 | lib | 13 | state: activityListeners: Set, let pollTimer, progress: WeakMap, agentStatusByKey: Map |
...
| `ws.ts` | 164 | lib | 1 | emits: approval, question, init, resolved, inject_error, pong |
...
| `inject_error` | server/routes/api.ts, server/ws.ts | client/hooks/use-companion.ts, ios:ios/WSFrame.swift |
