...
Generated by archmap on 2026-09-16. Do not edit; regenerate with `bun run ~/.claude/tools/archmap/cli.ts .` (or `tools/archmap/cli.ts .` where vendored).
...
## server (bun-server, `server/`, 52 modules, cap 600)
...
| `routes/command.ts` | 183 | route-host | 1 | state: inFlight: Set, listCache: Map · routes: 2 |
...
| `inputLine()` | `lib/command-menu.ts` | `routes/command.ts#POST /api/command/suggest`, `routes/command.ts#POST /api/command/list` |
