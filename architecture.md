...
Generated by archmap on 2026-09-22. Do not edit; regenerate with `bun run ~/.claude/tools/archmap/cli.ts .` (or `tools/archmap/cli.ts .` where vendored).
...
## server (bun-server, `server/`, 53 modules, cap 600)
...
| `lib/command-list.ts` | 321 | lib | 16 |  |
...
| `routes/command.ts` | 283 | route-host | 1 | state: listCache: Map · routes: 2 |
...
| `lib/command-scrape.ts` | 153 | lib | 13 | state: flows: Map |
| `ws.ts` | 153 | lib | 1 | emits: approval, question, init, resolved, inject_error, pong |
...
| `wiring/dialogs.ts` | 96 | lib | 3 | emits: dialog, dialog_closed · listens: dialogWatcher.start |
| `lib/hook-common.ts` | 95 | lib | 7 |  |
| `companion-server.ts` | 89 | route-host | 1 | routes: 5 |
...
| `inputLine()` | `lib/command-menu.ts` | `lib/command-list.ts`, `routes/command.ts#POST /api/command/suggest`, `routes/command.ts#POST /api/command/list` |
