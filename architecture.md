...
## server (bun-server, `server/`, 63 modules, cap 600)
...
| `routes/api.ts` | 355 | route-host | 1 | emits: resolved, inject_error, super_auto · routes: 16 |
...
| `lib/submit-confirm.ts` | 284 | lib | 15 | state: watches: Set, paneLocks: Map |
...
| `ws.ts` | 165 | lib | 1 | emits: approval, question, init, resolved, inject_error, pong |
...
| `companion-server.ts` | 75 | route-host | 1 | routes: 5 |
...
| `InjectTarget()` | `lib/keyboard-inject.ts` | `lib/submit-confirm.ts`, `routes/hooks.ts` |
