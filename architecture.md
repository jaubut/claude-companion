...
## server (bun-server, `server/`, 50 modules, cap 600)
...
| `lib/transcript.ts` | 273 | lib | 10 | state: states: Map |
...
| `routes/command.ts` | 100 | route-host | 1 | state: inFlight: Set · route: POST /api/command/suggest |
| `lib/feed.ts` | 97 | lib | 8 | state: feedListeners: Set, feedResetListeners: Set |
| `lib/hook-common.ts` | 95 | lib | 7 |  |
| `companion-server.ts` | 88 | route-host | 1 | routes: 5 |
...
- `ios:ios` — `~/apps/claude companion/architecture.json` generated 2026-09-14, 54 modules
...
| `resolveSession()` | `lib/sessions.ts` | `routes/api.ts#POST /api/inject`, `routes/command.ts#POST /api/command/suggest`, `routes/dialogs.ts#POST /api/dialog/key`, `routes/dialogs.ts#POST /api/dialog/pick`, `routes/model.ts#POST /api/model/open`, `routes/model.ts#POST /api/model/set`, `routes/model.ts#POST /api/model/cancel`, `ws.ts` |
...
| `capturePane()` | `lib/tmux-pane.ts` | `routes/command.ts#POST /api/command/suggest`, `wiring/dialogs.ts`, `wiring/orchestrator.ts` |
